...
def load_data(train_data, val_data):
    try:
        with open(train_data, encoding='utf-8') as training_f:
            training = json.load(training_f)
        with open(val_data, encoding='utf-8') as valid_f:
            validation = json.load(valid_f)

        tra = []
        val = []
        for elt in training:
            if "text" in elt and "stars" in elt:
                tra.append((elt["text"].split(), int(elt["stars"]-1)))
        for elt in validation:
            if "text" in elt and "stars" in elt:
                val.append((elt["text"].split(), int(elt["stars"]-1)))

        if not tra or not val:
        ...
        print(f"Error: Could not find the data files at {train_data} or {val_data}")
        raise
...
if __name__ == "__main__":
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.005, momentum=0.9, weight_decay=0.001)  # Reduced learning rate and added weight decay
    ...
    for epoch in range(args.epochs):
        ...
        print("Validation accuracy for epoch {}: {}".format(epoch + 1, current_val_accuracy))
        ...
        if patience_counter >= patience:
            print(f"Early stopping triggered! Best validation accuracy: {best_val_accuracy}")
            break
